    ...
    <div className="relative w-full" ref={containerRef}>
      ...
      >
        <span className="truncate capitalize">
          {options.find((opt) => opt === value) || "user"}
        </span>
      ...
      >
        {options.map((option) => (
          <button
            key={option}
            type="button"
            className="w-full text-left px-4 py-3 text-lg hover:bg-emerald-50 hover:text-emerald-700 transition-all first:rounded-t-2xl last:rounded-b-2xl capitalize"
            onClick={() => {
              onChange(option);
              setIsOpen(false);
            }}
